...
def marketplace_negotiation(product_details, buyer_budget, seller_min_price, user_role='buyer', interactive=True):
    ...
    for round_num in range(max_rounds):
        ...
        print(f"\n🛍️  Buyer: {buyer_response.messages[-1]['content']}")
        ...
        print(f"\n💼 Seller: {seller_response.messages[-1]['content']}")
        ...
        if "accept" in response:
            ...
            if round_num == max_rounds - 1:
                final_result["message"] = "Negotiation failed - maximum rounds reached"
